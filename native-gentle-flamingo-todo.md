...
- [ ] T4.14 `obfusjack-test21` full completion + `TEST.jar Calc` 5-run median ≤ 20 ms (formerly T4.9). Includes `randomRuntimeStableTenRuns` 10-consecutive-run stability and the full 16-case `NativeObfuscationIntegrationTest` matrix. Perf bar inherits T4.0 + T4.11 cleanup; further generic optimizations may be required and must be documented as "applies to all translated methods" (no class-/method-/owner-specific intrinsic). _Status 2026-05-03: TEST `Calc` 5-run median = 16ms ✓; obfusjack `Platform threads` median 39ms ✓ (≤44ms); `Virtual threads` median 33ms ✓ (≤35ms); matrix-mul `Parallel`/`VThreads` median 2 ms ✓ (improved from prior 3-4 ms); matrix-mul `Seq` median 34 ms vs ≤25ms target ✗ — first perf-recovery slice landed: `NEKO_FORCE_INLINE` macro + kind-switched inline `neko_barrier_load_oop_field/_array` + inline `neko_barrier_pre/post_store_oop_field` collapsed the function-pointer barrier dispatch on G1/CardTable to identity, allowing the compiler to inline through the entire fused-array call chain. 10-run obfusjack stability: 10/10 ✓. R-inspect: 0 forbidden JNI calls, only allowed `(*vm)->GetEnv` in `JNI_OnLoad`. Remaining 9 ms gap requires a second generic slice (struct-by-value reason return for fused helpers, or hoisting the `g_hotspot.*` invariants into per-method-entry-time const replicas) — still no benchmark-/class-/owner-specific intrinsic permitted._
