...
- [ ] T4.14 `obfusjack-test21` full completion + `TEST.jar Calc` 5-run median ≤ 20 ms (formerly T4.9). Includes `randomRuntimeStableTenRuns` 10-consecutive-run stability and the full 16-case `NativeObfuscationIntegrationTest` matrix. Perf bar inherits T4.0 + T4.11 cleanup; further generic optimizations may be required and must be documented as "applies to all translated methods" (no class-/method-/owner-specific intrinsic). _Status 2026-05-03: TEST `Calc` 5-run median = 16ms ✓; obfusjack `Platform threads` median 35ms ✓ (≤44ms); `Virtual threads` median 34ms ✓ (≤35ms); matrix-mul `Parallel`/`VThreads` 4ms ✓ (prior wins kept); matrix-mul `Seq` median 59ms vs ≤25ms target ✗ — perf gap inherits the T3.20 → T4.0 regression; T4.0 structural fix is already deployed (env-offset eager publication via `g_neko_off_thread_jni_environment_for_check` + always-inline `neko_exception_check`), so the residual gap requires additional generic dispatcher-overhead reduction (no benchmark-/class-/owner-specific intrinsic permitted)._
