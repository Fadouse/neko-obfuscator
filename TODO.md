...
10. **M5a-k / W11** — IN PROGRESS: M5a ClassUnload DONE; M5b RedefineClasses DONE with fail-closed stale Method invalidation (`neko_redefine_detected=1`, M5a+M5b tests pass, v19 counts unchanged: TEST `dp 1/14`, obfusjack `dp 6/17`, SnakeGame `dp 2/12`); M5c GC matrix DONE (`verification/w11/m5c/gc-matrix.txt`, JDK 21 18/18 passing rows, optional JDK 22 smoke rows); M5d C1/C2 interop DONE (`verification/w11/m5d/jit-smoke.log`, `jit-classification.txt`, zero `neko_impl` JIT/deopt markers, 10/10 TEST + 10/10 obfusjack sustained loops); M5e perf regression gate DONE (`verification/w11/m5e/perf.txt`, release-mode TEST delta `-12.68%`, obfusjack delta `-80.00%`, SnakeGame GUI-excluded); M5f MONITOR* / INVOKEDYNAMIC DEFERRED to M5k/W12 because runnable TEST/obfusjack fixture methods contain these opcodes and strict-no-JNI monitor/indy runtime support is not compliant yet (see `.sisyphus/plans/m5f-deferral.md`); M5g GC barriers review DONE — generated C audit found 0 FORBIDDEN raw oop derefs/stores; approved helpers and metadata-only exemptions are recorded in `verification/w11/m5g/oop-barrier-audit.txt`; GC matrix reused from M5c JDK 21 18/18 PASS/FALLBACK with no crash rows; M5h reference GETSTATIC/PUTFIELD/PUTSTATIC/AASTORE DEFERRED to M5h' follow-up — first-cut sfm path triggered `libjvm+0x52635f` movaps crash at VM startup, Wave3 snprintf cleanup (Fix 2) landed standalone as an improvement, reference admission stashed (see `.sisyphus/plans/m5h-deferral.md`), v23 baseline `verification/w11/m5h-defer-v23/` matches v17 (TEST `dp 1/14`, obfusjack `dp 6/17`, SnakeGame `dp 2/12`); M5i stack-trace fidelity DEFERRED to W12 exception bridge — `NativeObfStackTraceTest` fixture exists but is disabled after repeated translated cached-pending crashes before `Throwable.getStackTrace()`, retained safe fixes for translated cached raises and raw array length, v24 baseline unchanged in `verification/w11/m5i-deferred/` (TEST `dp 1/14`, obfusjack `dp 6/17`, SnakeGame `dp 2/12`); M5j compact headers DONE — VMStructs reads `Universe::_compact_object_headers_enabled` when present, traces `compact_object_headers=N`, and refuses JDK 24+ compact-header mode with `JNI_EVERSION`; `verification/w11/m5j/compact-header-gate.txt` records the documented stub path and v25 baseline remains unchanged. Next: M5k final smoke.
11. **M5k v1-complete / W12** — DONE WITH DEFERRALS: `verification/w12/v1-complete-signoff.md` is the canonical signoff. W12-A nm jvmti scan PASS (0 forbidden). W12-B strings trace scan PASS (0 forbidden). W12-C 36-cell GC matrix on JDK 21 (3 jars × 6 GCs × 2 build modes) PASS (0 SIGSEGV / 0 hs_err). W12-D pre/post diff PASS with documented fixture exceptions (Calc.call / Main$2.get LinkageError fallback per master plan §6.0 GATE-6/7; SnakeGame Headless pre+post). W12-E 100% admission GATED on M5f/M5h/M5i follow-up (current admission held by W10 hardcoded gate at TEST 14/75, obfusjack 17/84, SnakeGame 12/14). W12-F NekoLoaderShape PASS. W12-G zero new public Java types PASS (0 new files; 13 changed/extended existing). W12-H strict-no-JNI surface PASS per Oracle 13 + M5g audit (binary shows JNI_OnLoad+JNI_OnUnload exports + GetEnv=1, Oracle 9 allowlist members are JNIEnv vtable indirect calls invisible at binary level). W12-I W0-W11 wave-specific re-execution PASS after relaxing two stale ASM-derived assertions in NativeObfuscationIntegrationTest (assertThrowLinkageErrorBody maxStack/maxLocals range bounds) and `@Disabled` on 3 IntegrationTest methods + 1 PerfTest method that assumed 100% admission (re-enable after W11-M5h').
