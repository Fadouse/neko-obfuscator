...
| M5g | GC barriers review | DONE — Static audit found 0 FORBIDDEN raw oop derefs across 7 emitter Java files; 51 invocations of approved barrier-safe helpers (neko_load_oop_from_cell / neko_decode_klass_pointer / neko_class_klass_pointer / neko_method_const_method / neko_pending_exception family); 3 EXEMPT sites (Oracle 9 bootstrap allowlist + neko_throw_cached + neko_raise_cached_pending oop decode) all gated by JNI_OnLoad / cached-global path. | DONE — `verification/w11/m5g/oop-barrier-audit.txt` records audit verdict PASS. GC matrix REUSED from M5c (`verification/w11/m5c/gc-matrix.txt`) since audit found zero source change required. |
